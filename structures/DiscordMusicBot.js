...
class DiscordMusicBot extends Client {
  constructor(props) {
    ...
    if (this.botconfig.Token === "")
      return new TypeError(
        "Не заполнен файл botconfig.js. Пожалуйста, убедитесь, что ничего не заполнено, иначе бот не будет работать должным образом."
      );
    ...
    this.Manager = new Manager({
      ...
      .on("nodeConnect", (node) =>
        this.log(`Lavalink: Node ${node.options.identifier} подключена, работаем парни!`)
      )
      .on("nodeError", (node, error) =>
        this.log(
          `Lavalink: Node ${node.options.identifier} error какойта: ${error.message}`
        )
      )
      .on("trackStart", async (player, track) => {
        ...
        let TrackStartedEmbed = new MessageEmbed()
          ...
          .addField("Запрошено by", `${track.requester}`, true)
          .addField(
            "Продолжительность",
    ...
    fs.readdir(EventsDir, (err, files) => {
      if (err) this.log(err);
        ...
        files.forEach((file) => {
          ...
          this.logger.log("Event загружен: " + file.split(".")[0]);
        });
    });
  }
  ...
  sendError(Channel, Error) {
    let embed = new MessageEmbed()
      ...
      .setFooter(
        "Если вы считаете это ошибкой, пожалуйста, сообщите об этом на сервере поддержки!"
      );
