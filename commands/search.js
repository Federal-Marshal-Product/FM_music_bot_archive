  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (Searched.loadType == "NO_MATCHES")
      ...
      let msg = await message.channel.send(
        "**Введите номер песни, которую вы хотите воспроизвести! Истекает в `30 seconds`.**"
      );
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "**Вы слишком долго не отвечали. Выполните команду еще раз, если хотите поиграть во что-нибудь.!**"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(
          message.channel,
          "Пожалуйста, пришлите правильный идентификационный номер песни"
        );
      let Song = Searched.tracks[parseInt(SongIDmsg.content) - 1];
      if (!Song)
        return client.sendTime(
          message.channel,
          "Для данного идентификатора песня не найдена"
        );
      player.queue.add(Song);
      if (!player.playing && !player.paused && !player.queue.size)
        player.play();
      let SongAddedEmbed = new MessageEmbed();
      SongAddedEmbed.setAuthor(`Добавлена в очередь`, client.botconfig.IconURL);
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        switch (Searched.loadType) {
          ...
          case "TRACK_LOADED":
            ...
            return client.sendTime(
              interaction,
              `**Аддед ту очередь:** \`[${Searched.tracks[0].info.title}](${Searched.tracks[0].info.uri}}\`.`
            );
          ...
          case "PLAYLIST_LOADED":
            ...
            return client.sendTime(
              interaction,
              `**Плейлист добвален в очередь**: \n**${Searched.playlist.name}** \nЗаписано: **${Searched.playlistInfo.length} треков**`
            );
        }
      } else {
        try {
          ...
          return client.sendTime(
            interaction,
            `❌ | **Произошла ошибка при поиске:** ${err.message}`
          );
        }
        switch (res.loadType) {
          case "NO_MATCHES":
          ...
          case "PLAYLIST_LOADED":
            ...
            return client.sendTime(
              interaction,
              `**Плейлист добавлен в очередь**: \n**${res.playlist.name}** \nВ очереди: **${res.playlistInfo.length} треков**`
            );
          case "SEARCH_RESULT":
            let max = 10,
              collected,
            ...
            const resultss = new MessageEmbed()
              .setDescription(
                `${results}\n\n\t**Введите номер песни, которую вы хотите воспроизвести!**\n`
              )
              .setColor(client.botconfig.EmbedColor)
              .setAuthor(
            ...
            if (first.toLowerCase() === "cancel") {
              if (!player.queue.current) player.destroy();
              return awaitchannel.send("Отмененный поиск.");
            }

            const index = Number(first) - 1;
            if (index < 0 || index > max - 1)
              return awaitchannel.send(
                `Число, которое вы указали, больше или меньше общего числа поиска. Используйте - \`(1-${max})\``
              );
