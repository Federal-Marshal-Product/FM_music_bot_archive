...
module.exports = {
  name: "skipto",
  description: `Переход к песне в очереди`,
  usage: "<number>",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    try {
      if (!args[0])
        return client.sendTime(
          message.channel,
          `**Используй**: \`${GuildDB.prefix}skipto [number]\``
        );
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      try {
        if (!interaction.data.options)
          return client.sendTime(
            interaction,
            `**Чел юзай**: \`${GuildDB.prefix}skipto <number>\``
          );
