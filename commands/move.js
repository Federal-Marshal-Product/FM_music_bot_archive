...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!args[0] || !args[1])
      return client.sendTime(message.channel, "❌ | **Аргумент почему-то инвалид.**");

    // Check if (args[0] - 1) is a valid index
    let trackNum = parseInt(args[0] - 1);
    if (trackNum < 1 || trackNum > player.queue.length - 1) {
      return client.sendTime(message.channel, "❌ | **Почему номер трека инвалид?**");
    }
    ...
    if (dest < 1 || dest > player.queue.length - 1) {
      return client.sendTime(
        message.channel,
        "❌ | **Неверный пункт назначения дорожки**"
      );
    }
  ...
  SlashCommand: {
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!args[0].value || !args[1].value)
        return client.sendTime(interaction, "❌ | **Аргумент почему-то инвалид.**");

      // Check if (args[0] - 1) is a valid index
      let trackNum = parseInt(args[0].value - 1);
      if (trackNum < 1 || trackNum > player.queue.length - 1) {
        return client.sendTime(interaction, "❌ | **Почему номер трека инвалид?**");
      }
      ...
      if (dest < 1 || dest > player.queue.length - 1) {
        return client.sendTime(
          interaction,
          "❌ | **Недопустимое место назначения дорожки.**"
        );
      }
