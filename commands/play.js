...
module.exports = {
  name: "play",
  description: "Воспроизводите любимую песню.",
  usage: "[song]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    if (!message.member.voice.channel)
      return client.sendTime(
        message.channel,
        "❌ |**Для воспроизведения чего-либо вы должны находиться в голосовом канале.!**"
      );
    if (
      ...
      if (SearchString.match(client.Lavasfy.spotifyPattern)) {
        ...
        if (Searched.loadType === "PLAYLIST_LOADED") {
          ...
          if (!player.playing && !player.paused && !player.queue.size)
            player.play();
          SongAddedEmbed.setAuthor(`добавил в очередь`, client.botconfig.IconURL);
