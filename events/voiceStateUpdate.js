    ...
    case "JOIN":
      if (stateChange.members.size === 1 && player.paused) {
        let emb = new MessageEmbed()
          .setAuthor(`Возобновление приостановленной очереди`, client.botconfig.IconURL)
          .setColor(client.botconfig.EmbedColor)
          .setDescription(
            `Возобновляю воспроизведение, потому что вы все оставили меня в войсе с музыкой в одиночестве.`
          );
    ...
    case "LEAVE":
      if (stateChange.members.size === 0 && !player.paused && player.playing) {
        ...
        let emb = new MessageEmbed()
          ...
          .setColor(client.botconfig.EmbedColor)
          .setDescription(`Игрок был поставлен на паузу, потому что все ушли :(`);
        await client.channels.cache.get(player.textChannel).send(emb);
      }
      break;
  }
